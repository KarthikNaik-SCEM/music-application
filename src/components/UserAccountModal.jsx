...
const UserAccountModal = ({closeUserModal}) => {
    ...
    useEffect(() => {
        const fetchUser=async()=>{
            ...
            const json= await fetch("http://localhost:3000/getuser",{
                  method:"GET",
                  headers:{
    ...
    <div className='fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2  w-1/2 bg-white rounded-md drop-shadow-2xl'>
        ...
        <div className='flex flex-col gap-2'>
            ...
            <div className='flex gap-5 justify-evenly p-3'>
                ...
                <button className='font-bold text-sm py-1 px-3 border-2 border-gray-300 rounded-lg size-full' onClick={closeUserModal}>Exit</button>
            </div>
        </div>
    </div>
  )
